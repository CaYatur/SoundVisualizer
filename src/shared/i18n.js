...
(function () {
  ...
  const EN = {
    ...
    'Açıkken motor MilkDrop\'un kendi değerlerini kullanır: gürültü dokularının kafes ölçekleri, gerçekten üç boyutlu hacim gürültüsü, ekran boyunca değişen renk kayması, doğru bulanıklık ölçeği, ağın dikey yönü, warp titreşiminin kendi ölçeği ve hızı, dalga yumuşatma ve sese göre dalga saydamlığı, dış/iç kenarlıklar ve merkez karartma. Kapalı hâl motorun daha önceki yaklaşık değerlerini geri verir; presetler iki durumda da çalışır, yalnız görüntü farklıdır.': 'When on, the engine uses MilkDrop’s own values: the lattice scales of the noise textures, volume noise that really is three-dimensional, the hue shift that varies across the screen, the correct blur scale, the vertical direction of the mesh, the warp ripple’s own scale and speed, waveform smoothing and volume-driven waveform alpha, the outer and inner borders, and the centre darkening. Off restores the engine’s earlier approximate values; presets run either way, only the picture differs.',
