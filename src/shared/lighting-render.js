'use strict';
/* Işık renk hesabı — Windows Dynamic Lighting ve OpenRGB için ORTAK.
 *
 * Bu kod src/main/dynamic-lighting.js içindeydi ve yalnız Windows
 * LampArray'ini sürüyordu. OpenRGB eklenince aynı hesabın ikinci bir
 * uygulaması gerekecekti; iki uygulama zamanla birbirinden kayar ve aynı
 * sahnede iki farklı renk üretirdi. Onun yerine hesap buraya taşındı.
 *
 * Fabrika biçiminde: her tüketici createRenderer() ile KENDİ animasyon
 * durumunu alır. Ortak durum olsaydı iki tüketici aynı karede zamanlayıcıyı
 * iki kez ilerletir, dalgalar ve parlamalar hızlanırdı.
 *
 * Taşıma sırasında hiçbir satır değiştirilmedi ve bu doğrulandı:
 * tests/lighting-render.test.js, taşımadan ÖNCE alınan çıktı anlık
 * görüntüsüyle karşılaştırıyor. Fark çıkarsa renk hesabı bozulmuş demektir.
 */
(function () {
