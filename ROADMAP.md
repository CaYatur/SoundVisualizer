...
- **Waveforms read the newest audio** · done on `main`. The default and custom
  ...
  equal.
- **The bands read the aligned left channel.** Found with the item above:
  MilkDrop computes `bass`, `mid` and `treb` from the left channel after
  alignment (plugin.cpp:6875-6884); ours still read the mono mix, unaligned.
  Every preset reads the bands, so this gets its own measurement.
- **The default wave's modes draw as MilkDrop draws them.** Also found with the
  waveform item, in milkdropfs.cpp:2666-3100: mode 0 reads the right channel
  and mode 4 the left where we average them; mode 6 is a single line, where we
  draw mode 7's pair; modes 4, 6 and 7 cap their points at a third of the
  render width and read from the middle of the valid samples; modes 6 and 7
